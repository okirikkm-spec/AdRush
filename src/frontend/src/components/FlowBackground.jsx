...
void main() {
  ...
  // Толщина и спад ореола заданы в CSS-пикселях, поэтому линии одинаковы на любом экране.
  float lw   = mix(1.13, 6.65, blur) * uPx;
  float fall = mix(0.24, 0.056, blur) / uPx;
  ...
  // Полосу сглаживания не даём опускаться ниже пикселя буфера: у линии тоньше пикселя
  // край иначе ложится «ступеньками» и дрожит при движении поля.
  float w    = lw * (0.55 + 0.45 * heat);
  float core = 1.0 - smoothstep(0.0, max(w, 0.9), dist);
  float halo = exp(-dist * fall) + 0.15 * exp(-dist * fall * 0.16);
  // У горизонта изолинии сходятся плотнее пикселя — гасим, иначе мерцающая рябь.
  float dense = 1.0 - smoothstep(0.165, 0.525, gw * uPx);
...
/**
 * Буфер рендерится один к одному с экраном. Раньше здесь было 0.75: кадр рисовался
 * в три четверти разрешения, а браузер растягивал его — на изолиниях от этого
 * оставались лесенки, ведь диагональ сглаживается уже после потери пикселей.
 */
const BUFFER_SCALE = 1;
/** Потолок плотности: 2 покрывает Retina; на телефонах с 3x разница уже не видна. */
const MAX_DPR = 2;
...
export default function FlowBackground({ onFail }) {
  ...
  useEffect(() => {
    ...
    const resize = () => {
      ...
      // буферных пикселей на CSS-пиксель: от этого считаются толщина линий и порог плотности
      gl.uniform1f(u.uPx, q * BUFFER_SCALE * extra);
      return true;
    };
